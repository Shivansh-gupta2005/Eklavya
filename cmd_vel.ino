...
#define ROBOT_RADIUS 0.2  // meters (distance from center to wheel)
#define MAX_PWM 255
...
#define RPM_TO_PWM_SLOPE 0.5119
#define RPM_TO_PWM_INTERCEPT 3.10
...
int velocityToPWM(float velocity) {
  ...
  float rpm = fabs(velocity) * MAX_RPM;
  
  // Use the formula: PWM = (RPM + 3.10) / 0.5119
  int pwm = (rpm + RPM_TO_PWM_INTERCEPT) / RPM_TO_PWM_SLOPE;
...
void cmdVelCallback(const geometry_msgs::Twist& twist_msg) {
  ...
  float linear_x = twist_msg.linear.x;   // Forward/backward movement
  float linear_y = twist_msg.linear.y;   // Left/right movement
  float angular_z = twist_msg.angular.z; // Rotation

  // Mecanum wheel kinematics - calculate wheel velocities
  // Assuming the wheel numbering is:
  // 1: Front Left, 2: Front Right, 3: Rear Left, 4: Rear Right
  v1 = linear_x - linear_y - angular_z * ROBOT_RADIUS; // Front Left
  v2 = linear_x + linear_y + angular_z * ROBOT_RADIUS; // Front Right
  v3 = linear_x + linear_y - angular_z * ROBOT_RADIUS; // Rear Left
  v4 = linear_x - linear_y + angular_z * ROBOT_RADIUS; // Rear Right

  // Normalize velocities if any exceeds maximum
  float max_velocity = max(max(abs(v1), abs(v2)), max(abs(v3), abs(v4)));
  if (max_velocity > 1.0) {
    v1 /= max_velocity;
    v2 /= max_velocity;
    v3 /= max_velocity;
...
// Global variable for tracking last command time
...
void loop() {
  ...
  // Check for loss of communication (safety feature)
  if (millis() - last_cmd_time > 1000) { // If no command for 1 second
    stopmotors(); // Stop motors for safety
  }
